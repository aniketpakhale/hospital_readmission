---
title: "R Project - Diabetes Readmission"
author: "Aniket Pramod Pakhale"
...
##Loading the data
```{r}
raw.data <- read.csv("C:\\Users\\Fahim Usman\\Documents\\Edu\\Data Science\\R\\Project\\data\\diabetes_allcols.csv",sep = ",", header = T, na.strings = "?")
head(raw.data)
summary(raw.data)
```

###For getting the output, we dont need many columns hence we will be deselecting any irrelevant variables. We will also remove those columns which has more than 80% NAs.
```{r}
...
data2$diagnosis_group <- factor( rep("other",nrow(data2)),ordered = F, 
                                 levels = c("circulatory","respiratory","Digestive","Diabetes",
...
data2$diagnosis_3[data2$diag_3>=140 & data2$diag_3 <= 239 | data2$diag_3>=790 & 
                    ...
                    data2$diag_3==781 | data2$diag_3==782 | data2$diag_3==784] <- "Neoplasms"
...
##Rpart Decision Tree
###Without converting readmitted into a binary variable
#####Training  
```{r}
...
###By converting in a binary format  
#####Training  
```{r}
train$readmitted <- ifelse(train$readmitted == train$readmitted[1], 0, 1)
test$readmitted <- ifelse(test$readmitted == test$readmitted[1], 0, 1)
